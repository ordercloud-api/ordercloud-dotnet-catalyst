# OrderCloud.Integrations.Messaging.SendGrid

This project brings email to your ecommerce app using the [SendGrid API](https://docs.sendgrid.com/api-reference/mail-send/mail-send). Messaging can include marketing campaigns, batches, or single transactional messages send via email, SMS, or another channel. `SendGridService.cs` implements to the [`ISingleEmailSender`](./OrderCloud.Catalyst/Integrations/Interfaces/ISingleEmailSender.cs interface published in the base library ordercloud-dotnet-catalyst.
...
You will need these configuration data points to authneticate to the SendGrid API - *ApiKey*. Follow [these steps](https://docs.sendgrid.com/ui/account-and-settings/api-keys) to get API credentials. 

```c#
var sendGridService = new SendGridService(new SendGridConfig()
{
	ApiKey = "...",
});
```

For efficient use of compute resources and clean code, create 1 SendGridService object and make it available throughout your project using inversion of control dependency injection. 

```c#
services.AddSingleton<ISingeEmailSender>(sendGridService);
```

Notice that ISingeEmailSender is not specific to SendGrid. It is general to the problem domain and comes from the upstream ordercloud-dotnet-catalyst package. 

## Usage 

Inject the interfaces and use them within route logic. Rely on the interface whenever you can, not SendGridService. The layer of abstraction that ISingleEmailSender provides decouples your code from SendGrid as a specific provider and hides some internal complexity.
...
public class SendEmailService 
{
	...
	{
		// Inject interface. Implementation will depend on how services were registered, SendGridService in this case.
		_sender = sender; 
	}
...
This library also supports more complex cases that require mulitple merchant accounts with different credentials. For example, in a franchise business model where each location is independent but all sell on one ecommerce solution. In that case, still inject one instance of SendGridService exactly as above. You can provide empty strings for the credentials. However, when you call methods on the interfaces, provide the optional `overrideConfig` parameter. 

```c#
SendGridConfig overrideConfig = await FetchAccountCredentials(supplierID)
