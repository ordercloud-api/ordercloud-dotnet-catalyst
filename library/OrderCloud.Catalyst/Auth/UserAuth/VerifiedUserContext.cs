...
{	
	public class VerifiedUserContext
	{
		...
		{
			return token ?? throw new UserContextException("User token has not be verified. Add [OrderCloudUserAuth] on the controller route or call VerifyAsync(). Also make sure VerifiedUserContext and downstream services are request-scoped.");
		}
